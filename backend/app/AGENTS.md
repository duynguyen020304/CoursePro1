...
- **Models**: Extend Eloquent Model, define relationships, most use UUID primary keys
...
- **Traits**: Models use `HasAuditColumns` trait for `is_active`, soft deletes, timestamps
...
- Soft deletes via `HasAuditColumns` trait (provides `deleted_at`, `active()`, `notDeleted()` scopes)
- Models use UUID primary keys, including `Category.id`
