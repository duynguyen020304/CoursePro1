...
// --- Logic xác định đường dẫn và API_BASE (tương tự các file khác) ---
...
// --- Kết thúc logic đường dẫn ---

// --- Hàm callApi (cho API nội bộ) ---
...
function getYouTubeOEmbed(string $youtubeUrl): ?array
{
  ...
  $context_options = [
    ...
    'http' => [
      'timeout' => 10 // Timeout cho oEmbed request
    ]
  ];
...
// --- Kết thúc hàm YouTube oEmbed ---


$course_id_from_get = $_GET['courseID'] ?? null;
$lesson_id_from_get = $_GET['lessonID'] ?? null; // Để có thể chọn bài học cụ thể từ URL
$course_data = null;
$page_error = null;
$course_content_for_js = []; // Dữ liệu cho JavaScript
        ...
        if (isset($chapter['chapterLessons']) && is_array($chapter['chapterLessons'])) {
          foreach ($chapter['chapterLessons'] as $lesson) {
            $js_lesson = [
              ...
              'videoUrl' => null,
              'videoType' => null, // 'youtube' or 'local'
              'resources' => []
            ];
            if (!empty($lesson['lessonVideos']) && isset($lesson['lessonVideos'][0]['videoURL'])) {
              ...
              } else { // Giả định là video cục bộ
                $js_lesson['videoUrl'] = $videoUrl; // Tên file video
            ...
            if (!empty($lesson['lessonResources']) && is_array($lesson['lessonResources'])) {
              foreach ($lesson['lessonResources'] as $resource) {
                $js_lesson['resources'][] = [
                  ...
                  'path' => $resource['resourcePath'] // Tên file resource
                ];
              }
            }
...
// Xác định video/lesson ban đầu để phát
$initial_video_html = '<p class="text-center p-5">Chọn một bài học để bắt đầu.</p>';
$initial_lesson_id = null;
$initial_chapter_id_for_video = null; // Cần cho video cục bộ

if (!$page_error && !empty($course_content_for_js)) {
  if ($lesson_id_from_get) { // Nếu có lessonID từ URL
  ...
  // Nếu không có lessonID từ URL, hoặc lessonID không hợp lệ, lấy video đầu tiên
...
$instructorTitle = $course_data['instructors'][0]['biography'] ?? 'Giảng viên'; // Hoặc một trường khác nếu có
...
<body>
  ...
  <div class="course-main-layout">
    <div class="video-player-area">
      <div class="video-container ratio ratio-16x9 bg-black" id="videoPlayerWrapper">
        <?php echo $initial_video_html; // Hiển thị video ban đầu (YouTube iframe hoặc thẻ video) 
        ?>
      </div>
                      ...
                      <?php foreach ($chapter['lessons'] as $lesson): ?>
                        <li>
                          <a href="#" class="lesson-link <?php echo ($lesson['id'] === $initial_lesson_id) ? 'active-lesson' : ''; ?>"
                            ...
                            <?php if (!empty($lesson['videoUrl'])): // Giả định thời lượng nếu có video 
                            ?>
                            <?php endif; ?>
                          </a>
                        </li>
  ...
  <script>
    ...
    const youtubeOembedBaseUrl = "https://www.youtube.com/oembed?url="; // API endpoint của bạn
    ...
    function updateVideoPlayer(videoType, videoUrl, chapterId) {
      ...
      if (videoType === 'youtube' && videoUrl) {
        fetch(`${youtubeOembedBaseUrl}?url=${encodeURIComponent(videoUrl)}&format=json`)
          ...
          .then(data => {
            if (data.html) {
              // Tùy chỉnh iframe để responsive hơn
    ...
    lessonLinks.forEach(link => {
      link.addEventListener('click', function(e) {
        ...
        // Cập nhật URL để người dùng có thể chia sẻ hoặc bookmark bài học cụ thể
    ...
    // Kích hoạt tải tài liệu cho bài học đầu tiên (nếu có)
    if (currentActiveLessonElement) {
    ...
    // Cập nhật tổng số bài học (nếu cần)
