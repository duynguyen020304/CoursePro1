...
// Hàm callApi cục bộ cho trang này (nếu khác với header)
// Hoặc đảm bảo hàm callApi trong header được include và sử dụng đúng API_BASE_DETAIL
...
// Biến cho JavaScript
$js_api_base = API_BASE_DETAIL; // API_BASE_DETAIL đã được định nghĩa ở trên
$js_user_token = $_SESSION['user']['token'] ?? null;
$js_current_course_id = $course_data['courseID'] ?? null; // Chỉ lấy nếu $course_data tồn tại
        ...
        if (addToCartButton) {
            addToCartButton.addEventListener('click', async function() {
                ...
                try {
                    ...
                    if (cartData.success && cartData.cartID) { // API tạo cart mới
                        cartId = cartData.cartID;
                    } else if (cartData.success && cartData.data && cartData.cartID) { // API trả về cart đã có
                        cartId = cartData.cartID;
                    } else if (cartData.sucesss && cartData.cartID) { // Xử lý lỗi typo "sucesss" từ cart_api.php
                        cartId = cartData.cartID;
                    ...
                    if (addItemData.status === 'success') {
                        ...
                        location.reload(); // Tải lại trang để cập nhật số lượng trên header
                    } else {
...
<script src="<?php echo $app_root_url_for_paths; ?>/public/js/course-detail.js"></script> <?php // File JS này có thể chứa các hàm toggleSyllabusSection, toggleLessonDetails nếu bạn tách ra 
                                                                                            ?>
