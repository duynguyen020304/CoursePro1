...
DECLARE
    v_found_cartItemID  CARTITEM.CartItemID%TYPE;
    v_cartID            CARTITEM.CartID%TYPE;
    v_courseID          CARTITEM.CourseID%TYPE;
    v_quantity          CARTITEM.Quantity%TYPE;
    v_created_at        VARCHAR2(50);
BEGIN
    CART_PKG.GET_CART_ITEM_BY_ID('ITEM001', v_found_cartItemID, v_cartID, v_courseID, v_quantity, v_created_at);
    IF v_found_cartItemID IS NOT NULL THEN
        DBMS_OUTPUT.PUT_LINE('Item found for ITEM001:');
        DBMS_OUTPUT.PUT_LINE('  CartItemID: ' || v_found_cartItemID || ', CartID: ' || v_cartID || ', CourseID: ' || v_courseID || ', Quantity: ' || v_quantity);
    ELSE
        DBMS_OUTPUT.PUT_LINE('No item found for ITEM001.');
    END IF;
END;
/
...
DECLARE
    v_success BOOLEAN;
BEGIN
    CART_PKG.UPDATE_ITEM_QUANTITY('ITEM001', 5, v_success);
    IF v_success THEN DBMS_OUTPUT.PUT_LINE('Quantity for ITEM001 updated to 5.'); ELSE DBMS_OUTPUT.PUT_LINE('Failed to update ITEM001 quantity.'); END IF;

    CART_PKG.UPDATE_ITEM_QUANTITY('ITEM002', 0, v_success);
    IF v_success THEN DBMS_OUTPUT.PUT_LINE('ITEM002 deleted due to quantity 0.'); ELSE DBMS_OUTPUT.PUT_LINE('Failed to delete ITEM002.'); END IF;
END;
/
...
DECLARE
    v_success BOOLEAN;
BEGIN
    CART_PKG.DELETE_CART_ITEM('ITEM003', v_success);
    IF v_success THEN DBMS_OUTPUT.PUT_LINE('ITEM003 deleted successfully.'); ELSE DBMS_OUTPUT.PUT_LINE('Failed to delete ITEM003.'); END IF;
END;
/
...
DECLARE
    v_success BOOLEAN;
BEGIN
    CART_PKG.CLEAR_CART_ITEMS('CART001', v_success);
    IF v_success THEN DBMS_OUTPUT.PUT_LINE('All items cleared from CART001.'); ELSE DBMS_OUTPUT.PUT_LINE('Failed to clear items from CART001.'); END IF;
END;
/
