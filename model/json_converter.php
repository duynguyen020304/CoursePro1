<?php
// save as export-json-stream.php
// $instructors có thể là mảng lớn (hoặc bạn có thể sinh/đọc từng record từ DB)
$instructors = [
            [
...
// Tên file xuất
$filename = 'instructors-' . date('Ymd_His') . '.json';
...
foreach ($instructors as $item) {
    // Nếu cần loại bỏ trường nhạy cảm
    

    $part = json_encode($item, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
    if ($part === false) {
        // Nếu lỗi khi encode một phần, trả lỗi 500
        http_response_code(500);
        // ']' để giữ JSON hợp lệ nếu có thể
        echo ']';
        exit;
    }
